...
public class CCTV2RestController {
    ...
    public void turnOnCCTV2(WebDriver driver) throws InterruptedException {
        WebElement 교통감시CCTV버튼 = driver.findElement(By.cssSelector(".map_dep1_ul>li:nth-child(0)"));
        교통감시CCTV버튼.click();
        Thread.sleep(100);
    ...
    @GetMapping("/open/cctv2")
    public void openCCTV1(){
        System.out.println("GET /open/cctv2");


        log.info("GET /open/cctv2");
    ...
    @GetMapping("/snapshot/cctv2")
    public void test() {
        System.out.println("GET /snapshot/cctv2");
                    ...
                    if (!windowHandle.equals(mainWindowHandle)) {
                        ...
                        try {
                            ...
                            if (!cCTV2repository.existsByHlsAddr(popupURL)) {
                                ...
                                if (location != null) {
                                    cctv2.setLat(location.getLat());
                                    cctv2.setLon(location.getLng());
                                } else {
                                    // 부산시 중심 좌표 (부산시청 부근)
                                    cctv2.setLat(35.1796);
                                    cctv2.setLon(129.0756);
                                }

                                cctv2.setCategory("교통");
    ...
    @GetMapping(value = "/get/cctv2" ,produces = MediaType.APPLICATION_JSON_VALUE)
    public  List<CCTV2>  t1(){
        log.info("GET /get/cctv2....");
        return cCTV2repository.findAll();
    }

    @GetMapping(value="/search/cctv2",produces = MediaType.APPLICATION_JSON_VALUE)
